...
namespace GameEditor.Animation
{
    public class AnimGraphDrawer
    {
        ...
        private void DrawGraph(Rect area, AnimGraphLayout layout)
        {
            ...
            b.Expand(
                new Vector3(
                    s_DefaultMaximumNormalizedNodeSize,
                    s_DefaultMaximumNormalizedNodeSize,
                    0
                )
            );
        ...
        void DrawRect(Rect rect, Color color, string text, bool active, bool selected)
        {
            ...
            if (selected)
            {
                ...
                GUI.Box(
                    new Rect(rect.x - t, rect.y - t, rect.width + 2 * t, rect.height + 2 * t),
                    string.Empty,
                    m_NodeRectStyle
                );
            }
            ...
            if (active)
            {
                ...
                GUI.Box(
                    new Rect(
                        rect.x - s_ActiveNodeThickness,
                        rect.y - s_ActiveNodeThickness,
                        rect.width + 2 * s_ActiveNodeThickness,
                        rect.height + 2 * s_ActiveNodeThickness
                    ),
                    string.Empty,
                    m_NodeRectStyle
                );
        ...
        private static int ComputeFontSize(Vector2 nodeSize, string text)
        {
            ...
            int longuestWord = int.Parse(words.Max(s => s.Length));
        ...
        private static void GetTangents(
            Vector2 start,
            Vector2 end,
        ...
        )
        {
            ...
            startTangent =
                start + new Vector2((end.x - start.x) * weight + minTangent, 0) * cleverness;
            endTangent =
                end + new Vector2((end.x - start.x) * -weight - minTangent, 0) * cleverness;
        }
        ...
        static Vector2 ComputeNodeSize(Vector2 scale)
        {
            ...
            var nodeSize = new Vector2(
                s_DefaultMaximumNormalizedNodeSize * scale.x - extraTickness,
        ...
        private void PrepareNodeDatas(IEnumerable<AnimGraphLayout.Vertex> vertices)
        {
            ...
            foreach (var v in vertices)
            {
                ...
                var label = typeName.EndsWith("Node")
